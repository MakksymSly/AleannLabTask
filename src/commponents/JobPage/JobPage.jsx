      ...
      return (
        <div className="bg-white h-full tablet:pl-3">
          <div className="bg-white md:container md:mx-auto">
            ...
            <div>{/* Общий*/}
              <div>
                <div className="flex w-3/4 pt-8">
                  <div className="text-title text-2xl font-bold  ">{this.props.currentJob.title}</div>
                  <div className=" tablet:hidden desktop:flex text-title justify-end w-3/4">
                    <div className="mr-3">
                      <div className="font-bold text-xl flex"><img className="w-5 h-4 mr-2 self-center" src={euro} alt="euro" />{this.props.currentJob.salary}</div>
                      <div className="text-lg font-medium" >Brutto, per year</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="text-zinc-500 text-lg mt-2 mb-2 tablet:flex tablet:text-lg">Posted:{this.props.currentJob.createdAt.slice(0, 10)}
              ...
              <div className="text-lg text-title  w-3/4 pr-3 mb-12">{this.props.currentJob.description}</div>
            ...
            <div>{this.props.currentJob.benefits.map(b => <div key={b} className="flex items-center text-lg text-title  w-3/4 pr-3 "><img className="w-2 h-2" src={square} alt='img' />{b}</div>)}</div>
            ...
            <div>
              ...
              <div className="flex">{this.props.currentJob.employment_type.map(e => <div className="w-56 h-12 bg-etype text-etypetext rounded-xl border-title border-solid border-2 text-center mr-3 pt-2 font-bold " key={e}>{e}</div>)}</div>
              <div className="text-title text-lg mt-4 mb-3">Benefits</div>
              <div className="flex">{this.props.currentJob.benefits.map(b => <div key={b} className="w-56 h-12 bg-yellow-200/100 text-benefitsText rounded-xl border-benefitsText border-solid border-2 text-center mr-3 pt-2 font-bold ">{b}</div>)}</div>
              <div className="text-title text-2xl font-bold border-b-2 w-3/4 mt-14 mb-6">Attached images</div>
              <div className="flex mb-20">{this.props.currentJob.pictures.map(p => <img key={key++} className="h-40 w-60 rounded-2xl mr-5" src={p} alt="img" />)}</div>
            </div>
          ...
          <div className=" tablet:block m-auto  desktop:hidden   bg-map w-80 h-60 rounded-xl text-white">
            <div className="float-left pl-10 pt-7  ">
              <div className="font-bold text-xl">{this.props.currentJob.name}</div>
              <div className="items-center flex justify-center text-lg"><img className="w-3 h-3" src={pin} alt="img" />{this.props.currentJob.address}</div>
              <div className="text-lg">{this.props.currentJob.phone}</div>
